<template>
  <div style="min-height: 30vh;">
    <div class="trend-metric__chart" v-cloak v-for="(data,k,i) in serviceMap" :key="k">
      <div class="trend-metric__chart_title">
        ...
        <a
            ...
            @click="linkToDetail(data[0] && data[0].name)"
        >{{data[0] && data[0].name}}</a>-
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      _this.serviceMap = _.groupBy(_this.chunkDataSet[0], o => {
        return o.service;
      });
    },
    methods: {
      ...
      setMetricData(data) {
        let _this = this,
          dataList = [];
        if (!!data) {
          //整理画图数据形式为[[当前值],[最后值],[最近一天]]由于统计数据有些单位是时间单位、有写是次数、块数所以进行过滤判断
          dataList = [
            data.map(item => {
              return {
                time: item.time,
          ...
          return _this.reorganizeData(
            dataList, //对应画图数据
            `${data[0].service}-${data[0].name}`, //对应图表名称
